...
const getSpeed = (w: number, s: number, d?: boolean) => (0.5 + w) * s * (d ? -1 : 1)
...
export const createStars = (n: number, w: number, h: number, s: number, d?: boolean) => {
  const stars = []
  for (let i = 0; i < n; i += 1) {
...
export const updateStars = (stars: Star[], w: number, h: number, s: number, d?: boolean) => {
